            ...
            {logs.map((l) => (
              <tr key={l.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                ...
                <td className="px-4 py-3.5">
                  <Badge variant={
                    l.action.includes('LOGIN') ? 'info' :
                    l.action.includes('SEED') ? 'purple' :
                    l.action.includes('CREATE') || l.action.includes('ADD') ? 'success' :
                    l.action.includes('CANCEL') || l.action.includes('DELETE') ? 'danger' :
                    'default'
                  }>
                    {l.action}
                  </Badge>
