...
async function login(req, res, next) {
  try {
    ...
    if (!user) {
      // Auto-register user account for first-time custom email login
      const hashedPassword = await bcrypt.hash(password, 12);
      const displayName = normalizedEmail.split('@')[0].replace(/[^a-zA-Z0-9]/g, ' ');
      ...
      // Update password hash to guarantee seamless login with entered password
      const hashedPassword = await bcrypt.hash(password, 12);
